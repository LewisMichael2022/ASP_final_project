...
d = trainseq_varying_1; % 訓練序列僅為一段固定長度
x = data_varying_1;
...
for block_i = 1:num_blocks
    ...
    block_train_y = zeros(train_length, 1); % 訓練模式恢復的數據 
    ...
    for epoch=1:epoch_num
        ...
        for n = 1:train_length
            ...
            %RLS更新
            alpha = 1 / (lambda + buffer' * R_inverse * buffer);
            weights = weights + alpha * error' * R_inverse * buffer;
            R_inverse = (R_inverse - alpha * R_inverse * buffer * (buffer') * R_inverse) / lambda;
        end
    end
